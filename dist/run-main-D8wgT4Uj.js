import { a as normalizeEnv, cn as getPositiveIntFlagValue, dn as hasFlag, fn as hasHelpOrVersion, in as normalizeWindowsArgv, l as enableConsoleCapture, ln as getPrimaryCommand, on as getCommandPath, r as isTruthyEnvValue, sn as getFlagValue, un as getVerboseFlag, v as defaultRuntime } from "./entry.js";
...
import { Mt as loadDotEnv } from "./config-RlRKjsmM.js";
...
import "./subagent-registry-l8AgTmVw.js";
...
import { r as formatUncaughtError } from "./errors-Cu3BYw29.js";
...
import { u as installUnhandledRejectionHandler } from "./runner-C3vMtGzi.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-CZCd_MNM.js";
import { t as assertSupportedRuntime } from "./runtime-guard-92BlqYqn.js";
import { t as emitCliBanner } from "./banner-BdrLg_Ij.js";
import "./doctor-config-flow-BrEoEnAn.js";
import { n as ensureConfigReady } from "./config-guard-JXs8JQIU.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-Dv7zGKj9.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-DYR5lGwb.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-DaOQRBYv.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-CEohgsvQ.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-DhVngroR.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-kZK1aRN5.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-kZK1aRN5.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-KHJPZTxf.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-KHJPZTxf.js").then((n) => n.t);
...
async function runCli(argv = process$1.argv) {
	...
	const { buildProgram } = await import("./program-D2rkap8_.js");
	...
	if (primary) {
		const { getProgramContext } = await import("./program-context-Hoz_7ZCt.js").then((n) => n.n);
		const ctx = getProgramContext(program);
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-ChcE5GEq.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-CX0RmPHw.js").then((n) => n.a);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { registerPluginCliCommands } = await import("./cli-DICddb54.js");
		const { loadConfig } = await import("./config-RlRKjsmM.js").then((n) => n.t);
