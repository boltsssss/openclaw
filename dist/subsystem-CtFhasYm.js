import { a as resolveOAuthDir, c as expandHomePrefix, l as resolveEffectiveHomeDir, n as resolveConfigPath, u as resolveRequiredHomeDir } from "./paths-BY8fKpqm.js";
...
function resolvePreferredOpenClawTmpDir(options = {}) {
	...
	const resolveDirState = (candidatePath, requireWritableAccess) => {
		try {
			if (!isTrustedPreferredDir(lstatSync(candidatePath))) return "invalid";
			if (requireWritableAccess) accessSync(candidatePath, fs.constants.W_OK | fs.constants.X_OK);
			return "available";
		} catch (err) {
	...
	const ensureTrustedFallbackDir = () => {
		...
		if (resolveDirState(fallbackPath, true) !== "available") throw new Error(`Unsafe fallback OpenClaw temp dir: ${fallbackPath}`);
		return fallbackPath;
	};
	...
	try {
		...
		if (resolveDirState(POSIX_OPENCLAW_TMP_DIR, true) !== "available") return ensureTrustedFallbackDir();
